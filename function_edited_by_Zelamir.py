...
class Q_Learning:
    ...
    def __init__(self,env,alpha,gamma,epsilon,numberEpisodes,numberOfBins,lowerBounds,upperBounds):
        ...
        # this list stores sum of rewards through all episodes (added by Zelamir)
        self.sumRewardsCumulative=[]
    ...
    def simulateEpisodes(self):
        ...
        for indexEpisode in range(self.numberEpisodes):
            ...
            ####### (Added/edited by Zelamir) ##################
            reward_int = int(np.sum(rewardsEpisode))
         
            print("Sum of rewards {}".format(reward_int))        
            self.sumRewardsEpisode.append(np.sum(reward_int))

            cumulative_reward += np.sum(reward_int)
